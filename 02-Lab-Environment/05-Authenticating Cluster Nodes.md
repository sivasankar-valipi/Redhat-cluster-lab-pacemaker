...
Unlike a normal Linux user, it is not intended for interactive logins. Instead, it is used internally by the pcsd daemon to authenticate and authorize cluster management operations.
...
***Important: The password must be identical on every cluster node***
...
The command prompts for the hacluster password.
